...
function Weapon:getValueByLocationOffset(offset)
    return rom.readbyte(memory_start + self.ramLocation * 6 + offset)
end

function Weapon:getName ()
    return weapons[self.ramLocation]
end

function Weapon:getHitPercentage ()
    return self:getValueByLocationOffset(properties['hit_percentage'])
end

function Weapon:getDamage ()
    return self:getValueByLocationOffset(properties['damage'])
end
...
function Weapon:getSpellCast ()
    return self:getValueByLocationOffset(properties['spell_cast'])
end

function Weapon:getElement ()
    return elementals[self:getValueByLocationOffset(properties['element'])]
end
